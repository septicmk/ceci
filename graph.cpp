...
graph::graph(const char * graphFile)
{
    ...
    vert_max = 0;
    edge_count = 0; // 计数边
    label_count = 0; // 计数label数量
    size_t len = 0; // size of the initial buffer
    ssize_t read; // the num of characters stored in the buffer
...
//            label_count ++;
            ...
            { 
                if(line[i] == ' ')
                {
                    label_count ++; // 读到第2个空格就+1了，只能说明有label，忽略了label是什么
                }
            }
            if(v > vert_max)
                vert_max = v; // vert_max计数某个最大值？用v更新
        }
        else
            if(read > 0 && line[0] == 'e')
                edge_count ++;
